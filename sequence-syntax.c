...
object* stream(object* args, object* cont) {
	...
	object* eval_args = alloc_list_2(first, environment);
...
object* eval_and_map(object* args, object* cont) {
	...
	object* start_call = alloc_call(&map_proc, empty_list(), cont);
	object* start_cont = alloc_cont(start_call);
	
	object* eval_args = alloc_list_2(syntax, environment);
	object* eval_call = alloc_call(&eval_list_elements_proc, eval_args, start_cont);
...
object* eval_and_fold(object* args, object* cont) {
	...
	object* fold_call = alloc_call(&fold_proc, empty_list(), cont);
...
object* eval_and_filter(object* args, object* cont) {
	...
	object* start_call = alloc_call(&filter_proc, empty_list(), cont);
	object* start_cont = alloc_cont(start_call);
